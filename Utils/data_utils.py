...
class UDataset:
    ...
    def make_dev_split(self, dev_split=0.2):
        self.__train, self.__dev = np.split(
            self.__train, [int((1-dev_split)*len(self.__train)),])
...
class Dataset:
    ...
    def load_csv(self, train_path, label_name, separator, test_path=None, columns=None, dev_path=None):
        self.__train = pd.read_csv(
            train_path, sep=separator, header=0, usecols=columns)
        self.__label_name = label_name
        if test_path is not None:
            self.__test = pd.read_csv(
                test_path, sep=separator, header=0, usecols=columns)
        if dev_path is not None:
            self.__dev = pd.read_csv(
                dev_path, sep=separator, header=0, usecols=columns)

    def parse_data(self, parse_col_name, parse_type):
        # example: dataset.parse_data(parse_col_name='review', parse_type='html.parser')
        if not self.get_all():
            raise Exception(
                "Try to load the dataset using load_* functions. train or test are NoneType")
        self.__set_all([df[parse_col_name].apply(lambda x: soup(
            x, parse_type).text) for df in self.get_all()], parse_col_name)

    def parse_to_word_list(self, parse_col_name=None, stop=False, alphanum_sep=False, blind_num=False, blind_character='#'):
        if stop:
            self.__set_all([[clear_stop_words(safe_word_tokenize(split_alphanum(x, alphanum_sep, blind_num, blind_character).lower(
            ))) for x in df[parse_col_name]] for df in self.get_all()], parse_col_name)
        else:
            self.__set_all([[safe_word_tokenize(split_alphanum(x, alphanum_sep, blind_num, blind_character).lower(
            )) for x in df[parse_col_name]] for df in self.get_all()], parse_col_name)

    def list_to_stemmer(self, stem_col_name):
        stemmer = SnowballStemmer("english")
        self.__set_all([[[stemmer.stem(x) for x in sample]
                       for sample in df[stem_col_name]] for df in self.get_all()], stem_col_name)

    def list_to_lemma(self, lemma_col_name):
        lemmatizer = WordNetLemmatizer()
        self.__set_all([[[lemmatizer.lemmatize(x) for x in sample]
                       for sample in df[lemma_col_name]] for df in self.get_all()], lemma_col_name)

    def replace(self, parse_col_name=None, old=None, new=None):
        self.__set_all([[replace_character(sentence, old, new)
                       for sentence in df[parse_col_name]] for df in self.get_all()], parse_col_name)
    ...
    def save_pickle(self, train_path, test_path=None, dev_path=None):
        if self.__train is None:
            raise Exception("Train are NoneType")

        self.__train.to_pickle(train_path)
        if self.__test is not None and test_path is not None:
    ...
    def make_dev_split(self, dev_split=0.2):
        if self.__dev is None and self.__train is not None:
            ...
            Xtrain, Xdev, ytrain, ydev = train_test_split(
                self.get_train_x(), self.get_train_y(), test_size=dev_split, random_state=42)
            self.__train = pd.concat(
                [Xtrain, ytrain], axis=1).reset_index(drop=True)
            self.__dev = pd.concat([Xdev, ydev], axis=1).reset_index(drop=True)
        else:
            raise Exception(
                "self.__train is None or self.__dev is already set")

    def classes_distribution(self, condition='columns'):
        elements = [x[eval("x."+condition)] for x in self.get_all()]
        tuples = [list(zip(*[[x, y] for x, y in df[self.__label_name].value_counts().items()]))
                  for df in elements]
        ...
        for i in range(len(tuples)):
            ...
            plt.ylabel('Rating')
            plt.title("Class distribution in "+sets[i]+" dataset")
    ...
    def text_len_distribution(self, col_name, plot=False):
        ...
        if plot:
            ...
            values, base = np.histogram(data, bins=(
                np.max(data)-np.min(data)), density=True)
    ...
    def __set_all(self, newdata, column):
        datasets = [self.__train, self.__test, self.__dev]
    ...
    def set_column(self, newdata, column):
        if isinstance(newdata, pd.DataFrame) and column in {"train", "test", "dev"}:
            if column == "train":
                self.__train = newdata
            elif column == "test":
                self.__test = newdata
            else:
                self.__dev = newdata
...
def safe_word_tokenize(text):
    """Fallback to a regex-based tokenizer when punkt resources are unavailable."""
    try:
